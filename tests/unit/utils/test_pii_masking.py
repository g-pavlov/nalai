...
class TestPIIMasking:
    ...
    @pytest.mark.parametrize("value,pii_type,expected", [
    ...
    @pytest.mark.parametrize("pii_type,expected", [
    ...
    def test_mask_pii_disabled(self, mock_settings, pii_type, expected):
        ...
        result = mask_pii("john.doe@example.com" if pii_type == "email" else "John Smith" if pii_type == "name" else "testvalue", pii_type)
        expected_unmasked = "john.doe@example.com" if pii_type == "email" else "John Smith" if pii_type == "name" else "testvalue"
        assert result == expected_unmasked

    @pytest.mark.parametrize("data,pii_fields,expected_emails,expected_names", [
        (
            {"email": "john.doe@example.com", "name": "John Smith", "user_id": "user-123", "other_field": "not_pii"},
            None,
            "jo***@example.com",
            "J*** S***"
        ),
    ...
    def test_mask_dict_pii(self, mock_settings, data, pii_fields, expected_emails, expected_names):
    ...
    @pytest.mark.parametrize("metadata,expected_email,expected_given,expected_family", [
        (
            {
    ...
    def test_mask_audit_metadata(self, mock_settings, metadata, expected_email, expected_given, expected_family):
    ...
    @pytest.mark.parametrize("field_name,is_pii", [
        ("email", True),
        ("user_email", True),
        ("given_name", True),
        ("phone_number", True),
...
class TestPIIMaskingIntegration:
    ...
    @pytest.mark.parametrize("audit_metadata,expected_masked", [
        (
            {
    ...
    def test_complete_pii_masking_workflow(self, mock_settings, audit_metadata, expected_masked):
    ...
    @pytest.mark.parametrize("masking_config,test_value,pii_type,expected", [
        ({"audit_mask_pii": True, "audit_mask_emails": True}, "john.doe@example.com", "email", "jo***@example.com"),
        ({"audit_mask_pii": True, "audit_mask_names": True}, "John Smith", "name", "J*** S***"),
        ({"audit_mask_pii": True, "audit_mask_ip_addresses": True}, "192.168.1.100", "ip_address", "192.168.1.***"),
        ({"audit_mask_pii": True, "audit_mask_emails": False}, "john.doe@example.com", "email", "john.doe@example.com"),
        ({"audit_mask_pii": True, "audit_mask_names": False}, "John Smith", "name", "John Smith"),
        ({"audit_mask_pii": True, "audit_mask_ip_addresses": False}, "192.168.1.100", "ip_address", "192.168.1.100"),
        ({"audit_mask_pii": False}, "john.doe@example.com", "email", "john.doe@example.com"),
        ({"audit_mask_pii": False}, "John Smith", "name", "John Smith"),
        ({"audit_mask_pii": False}, "192.168.1.100", "ip_address", "192.168.1.100"),
    ])
    def test_pii_masking_configuration_control(self, mock_settings, masking_config, test_value, pii_type, expected):
