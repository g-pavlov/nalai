...
class TestThreadAccessControl:
    ...
    def test_unsupported_backend(self):
        ...
        with pytest.raises(
            ValueError, match="Unsupported thread access control backend"
        ):
...
class TestThreadAccessControlIntegration:
    ...
    async def test_multiple_users_thread_isolation(self):
        ...
        user1_access_after = await access_control.validate_thread_access(
            user1_id, thread_id
        )
    ...
    async def test_concurrent_access_simulation(self):
        ...
        for _ in range(10):
            result = await access_control.validate_thread_access(
