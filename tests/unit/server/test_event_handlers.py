...
class TestSerializeEventDefault:
    ...
    @pytest.mark.parametrize("event,expected", [
        ({"key": "value", "nested": {"inner": "data"}}, {"key": "value", "nested": {"inner": "data"}}),
...
class TestFormatSseEventDefault:
    ...
    @pytest.mark.parametrize("event_type,data,expected", [
        ("test_event", '{"key": "value"}', "event: test_event\ndata: {\"key\": \"value\"}\n\n"),
        ("empty_event", "", "event: empty_event\ndata: \n\n"),
        ("special_event", '{"message": "Hello\\nWorld\\twith\\rspecial chars"}', 
         "event: special_event\ndata: {\"message\": \"Hello\\nWorld\\twith\\rspecial chars\"}\n\n"),
    ])
...
class TestProcessAndFormatEvent:
    ...
    @pytest.mark.parametrize("event,allowed_events,expected_result,should_call_serialize,should_call_format", [
        (
        ...
        (
            {
                ...
                "data": {
                    "chunk": {
                        "__interrupt__": [{"value": "interrupt_data"}]
                    }
                }
            },
    ...
    def test_process_and_format_event(
        self, mock_serialize_event, mock_format_sse_event, 
        ...
        result = process_and_format_event(event, allowed_events, mock_serialize_event, mock_format_sse_event)
        ...
        if should_call_format:
            mock_format_sse_event.assert_called_once_with("data", '{"serialized": "data"}')
        else:
...
class TestStreamInterruptableEvents:
    ...
    @pytest.mark.parametrize("agent_events,has_interrupts,expected_event_count,should_raise_exception", [
        (
            [
        ...
        (
            ["not_a_dict_event", {"event": "on_chat_model_stream", "data": {"content": "test"}}],
            False,
    ...
    async def test_stream_interruptable_events(
        self, mock_agent, mock_serialize_event, mock_format_sse_event,
        mock_config, resume_input, agent_events, has_interrupts, expected_event_count, should_raise_exception
    ):
        ...
        if should_raise_exception:
            ...
            if has_interrupts:
                ...
                mock_snapshot.__getitem__.return_value.__getitem__.return_value.interrupts = [MagicMock()]
            else:
        ...
        if should_raise_exception:
            ...
            async for event in stream_interruptable_events(
                mock_agent, resume_input, mock_config,
    ...
    @pytest.mark.parametrize("allowed_events", [
        ["on_chain_start"],
        ["on_chat_model_stream", "on_tool_end"],
        [],
    ])
    ...
    async def test_stream_interruptable_events_with_custom_allowed_events(
        self, mock_agent, mock_serialize_event, mock_format_sse_event,
        mock_config, resume_input, allowed_events
    ):
    ...
    async def test_stream_interruptable_events_interrupt_access_error(
        ...
        mock_snapshot.__getitem__.return_value.__len__.return_value = 1
        mock_snapshot.__getitem__.return_value.__getitem__.side_effect = IndexError("No interrupts")
        mock_agent.get_state.return_value = mock_snapshot
