...
from api_assistant.server.routes import create_basic_routes, create_agent_routes
from api_assistant.server.models import AgentInvokeRequest, AgentStreamEventsRequest, HumanReviewRequest
...
class TestBasicRoutes:
    @pytest.mark.parametrize("path,expected_status,expected_redirect", [
        ("/", 200, True),
        ("/healthz", 200, False),
    ])
...
class TestAgentInvoke:
    @pytest.mark.parametrize("input_messages,config,expected_status", [
        ([{"type": "human", "content": "hi"}], None, 200),
        ([], None, 422),  # Invalid: empty messages
    ])
    def test_agent_invoke(self, client, app_and_agent, input_messages, config, expected_status):
        ...
        with patch("api_assistant.server.routes.setup_runtime_config_with_access_control", new=AsyncMock(return_value=({}, "tid"))):
...
class TestAgentStreamEvents:
    ...
    def test_agent_stream_events(self, client, app_and_agent, input_messages, config, expected_status):
        _, mock_agent = app_and_agent
        ...
        with patch("api_assistant.server.routes.setup_runtime_config_with_access_control", new=AsyncMock(return_value=({}, str(uuid.uuid4())))):
...
class TestHumanReview:
    ...
    def test_human_review(self, client, app_and_agent, action, expected_status):
        ...
        with patch("api_assistant.server.routes.setup_runtime_config_with_access_control", new=AsyncMock(return_value=({}, thread_id))):
            with patch("api_assistant.server.routes.stream_interruptable_events", new=fake_stream_interruptable_events):
    ...
    def test_human_review_invalid_action(self, client, app_and_agent):
        ...
        with patch("api_assistant.server.routes.setup_runtime_config_with_access_control", new=AsyncMock(return_value=({}, thread_id))):
            with patch("api_assistant.server.routes.stream_interruptable_events", new=AsyncMock()):
                ...
                assert "ValidationError" in str(exc_info.value) or "validation error" in str(exc_info.value).lower() 