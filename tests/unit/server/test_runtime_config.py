...
from api_assistant.server.models.identity import UserContext, IdentityContext
...
class TestConfigHelpers:
    ...
    @pytest.mark.parametrize("input_config,expected_configurable,expected_result", [
        ({"configurable": {"key": "value"}}, {"key": "value"}, {"key": "value"}),
        ({"configurable": None}, {}, {}),
        ({}, {}, {}),
    ])
    def test_ensure_configurable(self, input_config, expected_configurable, expected_result):
...
class TestEnsureThreadIdExists:
    ...
    @pytest.mark.parametrize("input_config,expected_thread_id_exists", [
    ...
    @pytest.mark.parametrize("thread_id,should_raise", [
    ...
    def test_ensure_thread_id_exists_validate_uuid(self, thread_id, should_raise):
        ...
        if should_raise:
            ...
            result_config, result_thread_id = ensure_thread_id_exists(config, validate_uuid=True)
...
class TestAddAuthTokenToConfig:
    ...
    @pytest.mark.parametrize("auth_disabled,headers,expected_token,should_raise", [
        (True, {}, "dev-token", False),
        (False, {"Authorization": "Bearer test-token-123"}, "test-token-123", False),
        (False, {}, None, True),
        (False, {"Authorization": ""}, None, True),
    ])
    @patch("api_assistant.server.runtime_config.settings")
    def test_add_auth_token_to_config(self, mock_settings, mock_request, auth_disabled, headers, expected_token, should_raise):
...
class TestAddUserContextToConfig:
    ...
    @pytest.mark.parametrize("user_context_success,expected_values", [
        (True, {
    ...
    def test_add_user_context_to_config(self, mock_get_user_context, mock_request, user_context_success, expected_values):
...
class TestValidateThreadAccessAndScope:
    ...
    @pytest.mark.parametrize("has_thread_id,access_granted,expected_scoped_id,should_raise", [
    ...
    async def test_validate_thread_access_and_scope(
        self, mock_log_event, mock_get_access_control, mock_get_user_context,
        mock_request, mock_user_context, has_thread_id, access_granted, expected_scoped_id, should_raise
    ):
        ...
        mock_access_control.create_user_scoped_thread_id.return_value = expected_scoped_id
        ...
        if should_raise:
            ...
            result_config, user_scoped_thread_id = await validate_thread_access_and_scope(config, mock_request)
            assert user_scoped_thread_id == expected_scoped_id
            assert result_config["configurable"]["thread_id"] == expected_scoped_id

    @patch("api_assistant.server.runtime_config.get_user_context")
    @pytest.mark.asyncio
    async def test_validate_thread_access_no_user_context(self, mock_get_user_context, mock_request):
...
class TestDefaultFunctions:
    ...
    def test_default_modify_runtime_config(self, mock_add_user_context, mock_add_auth_token, mock_request):
        """Test default runtime configuration modification."""
        mock_add_auth_token.return_value = {"configurable": {"auth_token": "test-token"}}
        mock_add_user_context.return_value = {"configurable": {"user_id": "test-user"}}
        
        result = default_modify_runtime_config({}, mock_request)
    ...
    async def test_default_modify_runtime_config_with_access_control(
        ...
        mock_validate_access.return_value = ({"configurable": {"thread_id": "scoped-id"}}, "scoped-id")
        
        result_config, thread_id = await default_modify_runtime_config_with_access_control({}, mock_request)
