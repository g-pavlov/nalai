...
def test_cache_service():
    ...
    cache_service.set(
        [HumanMessage(content="create the product")], 
        "Product created"
    )
    cache_service.set(
    ...
    for message_content, response, tool_calls, similarity in similar:
    ...
    similar_lower = cache_service.find_similar_cached_responses("create a product", similarity_threshold=0.5)
    print(f"  With lower threshold (0.5), found {len(similar_lower)} similar responses:")
    for message_content, response, tool_calls, similarity in similar_lower:
        print(f"    - '{message_content}' (similarity: {similarity:.3f}): '{response}'")
