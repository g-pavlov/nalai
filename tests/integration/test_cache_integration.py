...
def test_cache_integration():
    ...
    def test_cache_hits(test_case: dict[str, Any]):
        ...
        for i, prompt in enumerate(prompts):
            ...
            if (
                "output" in response
                and "messages" in response["output"]
                and response["output"]["messages"]
            ):
                last_message = response["output"]["messages"][-1]
                content = last_message.get("content", "")
                # has_tool_calls = bool(last_message.get("tool_calls"))  # Unused
                ...
                if content and content.strip():
                    print("  ✅ Has content - will be cached")
                else:
                    print("  ⚠️  Empty content - won't be cached")
            else:
        ...
        if len(times) >= 2:
            ...
            for i, time_taken in enumerate(subsequent_times):
                if time_taken < first_time * 0.8:  # 20% faster threshold
                    cache_hits += 1
                    print(
                        f"  ✅ Request {i + 2}: Cache hit detected ({time_taken:.3f}s < {first_time * 0.8:.3f}s)"
                    )
                else:
                    print(
                        f"  ❌ Request {i + 2}: No cache hit ({time_taken:.3f}s >= {first_time * 0.8:.3f}s)"
                    )

            print(
                f"  📈 Cache hit rate: {cache_hits}/{len(subsequent_times)} ({cache_hits / len(subsequent_times) * 100:.1f}%)"
            )
        ...
        if "output" in response1 and "output" in response2:
            ...
            if messages1 and messages2:
                ...
                if content1 == content2 and second_time < first_time * 0.5:
                    print(
                        f"  ✅ Exact cache hit working! Speedup: {first_time / second_time:.2f}x"
                    )
                    return True
                else:
                    print(
                        f"  ❌ Exact cache hit not working. Speedup: {first_time / second_time:.2f}x"
                    )
                    return False
            else:
    ...
    try:
        ...
        print("=" * 60)
        print(
            f"Exact cache hits: {'✅ Working' if exact_cache_working else '❌ Not working'}"
        )

        if total_requests > 0:
            hit_rate = total_cache_hits / total_requests * 100
            print(
                f"Similarity cache hits: {total_cache_hits}/{total_requests} ({hit_rate:.1f}%)"
            )
