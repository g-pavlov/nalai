...
def test_no_cache_header():
    ...
    def make_request(prompt: str, no_cache: bool = False) -> dict[str, Any]:
        ...
        payload = {
            ...
            "config": {
                "model": {"name": "gpt-4.1", "platform": "openai"}
            }
        }
    ...
    assert time3 > time2, f"No-cache request ({time3:.2f}s) should be slower than cached request ({time2:.2f}s)"
    
    print(f"\n✅ No-cache header test passed!")
    print(f"   - Normal requests: {time1:.2f}s, {time2:.2f}s, {time4:.2f}s")
    print(f"   - No-cache request: {time3:.2f}s")
    print(f"   - Cache bypass confirmed: no-cache request was slower")
