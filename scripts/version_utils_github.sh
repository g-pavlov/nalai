...
# Version Management Utilities for GitHub Actions
# ==============================================
# This script provides version incrementing functions for semantic versioning
# Adapted from GitLab CI version for GitHub Actions
...
generate_backward_targeting_message() {
    local manual_version=$1
    local main_baseline=$2
    local context=${3:-$(get_github_context_info)}
...
# Function to get GitHub context info for versioning context
get_github_context_info() {
    if [ -n "$GITHUB_EVENT_NAME" ] && [ "$GITHUB_EVENT_NAME" = "pull_request" ]; then
        echo "PR-$GITHUB_EVENT_NUMBER"
    elif [ -n "$GITHUB_REF_NAME" ]; then
        echo "BRANCH-${GITHUB_REF_NAME//[^a-zA-Z0-9]/}"
    else
...
increment_version_with_conflict_detection() {
    ...
    local context=${2:-$(get_github_context_info)}
