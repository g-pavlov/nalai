...
class ThreadAccessControlBackend(ABC):
    ...
    @abstractmethod
    async def create_thread(self, user_id: str, thread_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
...
class InMemoryThreadAccessControl(ThreadAccessControlBackend):
    ...
    async def validate_thread_access(self, user_id: str, thread_id: str) -> bool:
        ...
        if is_owner:
            ...
            logger.warning(f"User {user_id} denied access to thread {thread_id} (owner: {ownership.user_id})")
    ...
    async def create_thread(self, user_id: str, thread_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        if thread_id in self._thread_ownership:
            ...
            if existing_owner != user_id:
                raise ValueError(f"Thread {thread_id} already owned by user {existing_owner}")
    ...
    async def list_user_threads(self, user_id: str) -> List[ThreadOwnership]:
        """List all threads owned by a user."""
        user_threads = [
            ownership for ownership in self._thread_ownership.values()
    ...
    async def delete_thread(self, user_id: str, thread_id: str) -> bool:
        ...
        if ownership.user_id != user_id:
            logger.warning(f"User {user_id} cannot delete thread {thread_id} (owner: {ownership.user_id})")
            return False
...
class ThreadAccessControl:
    ...
    async def create_thread(self, user_id: str, thread_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
    ...
    async def get_thread_ownership(self, thread_id: str) -> Optional[ThreadOwnership]:
...
_thread_access_control: Optional[ThreadAccessControl] = None
