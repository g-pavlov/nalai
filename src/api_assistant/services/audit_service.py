...
class AuditBackend(ABC):
    ...
    @abstractmethod
    async def log_access(self, user_id: str, resource: str, action: str, metadata: Optional[Dict[str, Any]] = None, success: bool = True) -> None:
    ...
    @abstractmethod
    async def get_user_events(self, user_id: str, limit: int = 100) -> List[AccessEvent]:
...
class InMemoryAuditBackend(AuditBackend):
    ...
    def __init__(self, max_entries: int = 10000):
        ...
        logger.debug(f"In-memory audit backend initialized with max_entries={max_entries}")
    
    async def log_access(self, user_id: str, resource: str, action: str, metadata: Optional[Dict[str, Any]] = None, success: bool = True) -> None:
        """Log an access event with individual parameters."""
        logger.debug(f"Creating AccessEvent with user_id={user_id}, resource={resource}, action={action}, success={success}, metadata={metadata}")
...
class AuditService:
    ...
    async def log_access(self, user_id: str, resource: str, action: str, metadata: Optional[Dict[str, Any]] = None, success: bool = True, ip_address: Optional[str] = None, user_agent: Optional[str] = None, session_id: Optional[str] = None, request_id: Optional[str] = None) -> None:
        """Log an access event with identity-resource-action-time granularity."""
        # Add additional metadata if provided
        full_metadata = metadata or {}
        if ip_address:
            # Mask IP address if configured
            masked_ip = mask_pii(ip_address, "ip_address") if settings.audit_mask_ip_addresses else ip_address
        ...
        await self.backend.log_access(user_id, resource, action, full_metadata, success=success)
    
    async def log_thread_access(self, user_id: str, thread_id: str, action: str, 
                               success: bool = True, metadata: Optional[Dict[str, Any]] = None,
                               ip_address: Optional[str] = None, user_agent: Optional[str] = None,
                               session_id: Optional[str] = None, request_id: Optional[str] = None) -> None:
    ...
    async def log_api_access(self, user_id: str, api_endpoint: str, action: str,
                           success: bool = True, metadata: Optional[Dict[str, Any]] = None,
                           ip_address: Optional[str] = None, user_agent: Optional[str] = None,
                           session_id: Optional[str] = None, request_id: Optional[str] = None) -> None:
    ...
    async def log_request_start(self, user_id: str, method: str, path: str,
                              ip_address: Optional[str] = None, user_agent: Optional[str] = None,
                              session_id: Optional[str] = None, request_id: Optional[str] = None) -> None:
    ...
    async def log_request_complete(self, user_id: str, method: str, path: str, status_code: int,
                                 duration: float, ip_address: Optional[str] = None, 
    ...
    async def get_thread_events(self, thread_id: str, limit: int = 100) -> List[AccessEvent]:
...
async def log_access_event(user_id: str, resource: str, action: str, 
                          metadata: Optional[Dict[str, Any]] = None,
                          success: bool = True,
                          **kwargs) -> None:
    """Log an access event using the global audit service."""
    audit_service = get_audit_service()
    await audit_service.log_access(user_id, resource, action, metadata, success=success, **kwargs)
