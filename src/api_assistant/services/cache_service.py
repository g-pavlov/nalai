...
class CacheEntry:
    ...
    original_message: str = ""  # Store the original message content for similarity matching
...
class TokenSimilarityMatcher:
    ...
    def __init__(self):
        ...
        self.verbs = {
            'create', 'add', 'make', 'generate', 'build', 'new',
            'list', 'get', 'show', 'display', 'find', 'search',
        ...
        self.nouns = {
            'product', 'user', 'order', 'item', 'data', 'info',
            'list', 'details', 'schema', 'structure', 'config',
    ...
    def _weighted_jaccard_similarity(self, tokens1: Dict[str, float], tokens2: Dict[str, float]) -> float:
    ...
    def _is_likely_false_positive(self, intent1: str, intent2: str) -> bool:
        ...
        opposites = [
            ("create", "delete"), ("add", "remove"), ("get", "delete"),
            ("list", "delete"), ("show", "hide"), ("enable", "disable"),
            ("activate", "deactivate"), ("new", "delete"), ("make", "destroy")
        ]
        ...
        for opp1, opp2 in opposites:
            if (opp1 in words1 and opp2 in words2) or (opp2 in words1 and opp1 in words2):
                return True
...
class CacheService:
    ...
    def get(self, messages: List[BaseMessage]) -> Optional[Tuple[str, List[Dict[str, Any]]]]:
        ...
        # Increment hit count
        entry.hit_count += 1
        logger.info(f"Cache hit for messages: {message_key[:8]}... (hits: {entry.hit_count})")
        
        return entry.response, entry.tool_calls
    
    def find_similar_cached_responses(self, message_content: str, 
                                    similarity_threshold: float = 0.8) -> List[Tuple[str, str, List[Dict[str, Any]], float]]:
        """
        ...
        for message_key, entry in self._cache.items():
            if not entry.is_expired() and entry.original_message:
                ...
                similarity = self.similarity_matcher.similarity(message_content, entry.original_message)
                
                if similarity >= similarity_threshold:
                    similar_contents.append((entry.original_message, entry.response, entry.tool_calls or [], similarity))
    ...
    def set(self, messages: List[BaseMessage], response: str, 
            tool_calls: Optional[List[Dict[str, Any]]] = None, 
...
def get_cache_service() -> CacheService:
    ...
    if _cache_service is None:
        ...
        _cache_service = CacheService(
            ...
            default_ttl_hours=settings.cache_ttl_hours
        )
