...
class InMemoryCacheBackend(CacheBackend):
    ...
    def __init__(self, max_size: int = 1000, default_ttl_hours: int = 1):
        ...
        logger.debug(
            f"In-memory cache backend initialized with max_size={max_size}, ttl={default_ttl_hours}h"
        )
    ...
    async def set(self, key: str, value: Any, ttl_seconds: int | None = None) -> None:
        ...
        if key in self._cache:
            ...
            self._cache[key].update(
                {"value": value, "expires_at": expires_at, "updated_at": datetime.now()}
            )
        else:
            # Check size limit
...
class IdentityAwareCache:
    ...
    def __init__(self, backend: str = "memory", config: dict[str, Any] | None = None):
        ...
        if backend == "memory":
            ...
            self.backend = InMemoryCacheBackend(
                max_size=max_size, default_ttl_hours=ttl_hours
            )
        elif backend == "redis":
    ...
    async def clear_user_cache(self, user_id: str) -> None:
        ...
        if isinstance(self.backend, InMemoryCacheBackend):
            ...
            keys_to_delete = [
                key for key in self.backend._cache.keys() if key.startswith(user_prefix)
            ]
            for key in keys_to_delete:
                await self.backend.delete(key)
            logger.info(
                f"Cleared {len(keys_to_delete)} cache entries for user {user_id}"
            )
        else:
...
_identity_aware_cache: IdentityAwareCache | None = None
...
async def get_cached_value(user_id: str, key: str) -> Any | None:
...
async def set_cached_value(
    user_id: str, key: str, value: Any, ttl_seconds: int | None = None
) -> None:
...
def get_cache_service():
    ...
    if _cache_service is None:
        ...
        _cache_service = IdentityAwareCache(
            backend="memory", config={"max_size": 1000, "ttl_hours": 1}
        )
...
async def cache_get(user_id: str, key: str) -> Any | None:
...
async def cache_clear(user_id: str | None = None) -> None:
